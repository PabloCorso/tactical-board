...
Player-owned overrides are stored as ordinary optional Player fields rather than in a separate override object. For example, if a grouped Player omits `props.color`, it inherits the Player Group color; if it sets `props.color`, that value is the Player-specific override. Geometry follows the same rule using the existing Board Object field: an omitted `size` inherits the Player Group size, while a present `size` is the Player-specific override. There is no parallel `sizeOverride`, `styleOverride`, or override container. This keeps the serialized Board shape close to the Figma-like inheritance model without introducing a second parallel style representation.
